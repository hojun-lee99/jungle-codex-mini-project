...
- 배포 고려: 환경변수 기반 설정과 `gunicorn` 지원으로 AWS, Render 같은 PaaS에 바로 올릴 수 있는 구조
...
1. GitHub 저장소에 최신 코드를 push
2. Render Dashboard에서 Blueprint 배포 열기
3. `SECRET_KEY` 값을 입력
4. MongoDB Atlas를 쓰지 않는 데모 배포라면 `USE_MOCK_ON_FAILURE=true` 상태로 바로 배포
5. 운영 배포라면 Render 환경변수에 `MONGO_URI`, `MONGO_DB_NAME`을 추가
